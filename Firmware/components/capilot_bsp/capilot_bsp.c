...
/* I2C master bus handle, stored after i2c_init */
static i2c_master_bus_handle_t s_bus_handle = NULL;

/* Cached device handles for known devices on the bus */
...
/* 设备句柄缓存（避免每次交易都创建/删除句柄） */
#define CAPILOT_BSP_MAX_I2C_DEVICES  4
static struct {
    uint16_t addr;
    i2c_master_dev_handle_t handle;
} s_dev_cache[CAPILOT_BSP_MAX_I2C_DEVICES];
static int s_dev_cache_cnt = 0;

/* 从缓存获取或新建设备句柄 */
static esp_err_t i2c_get_cached_dev(uint8_t dev_addr, i2c_master_dev_handle_t *out)
{
    for (int i = 0; i < s_dev_cache_cnt; i++) {
        if (s_dev_cache[i].addr == dev_addr) {
            *out = s_dev_cache[i].handle;
            return ESP_OK;
        }
    }
    if (s_dev_cache_cnt >= CAPILOT_BSP_MAX_I2C_DEVICES) {
        ESP_LOGE(TAG, "I2C device cache full (max %d)", CAPILOT_BSP_MAX_I2C_DEVICES);
    ...
    esp_err_t ret = i2c_master_bus_add_device(s_bus_handle, &dev_cfg, &h);
    if (ret != ESP_OK) return ret;
    s_dev_cache[s_dev_cache_cnt].addr   = dev_addr;
    s_dev_cache[s_dev_cache_cnt].handle = h;
    s_dev_cache_cnt++;
    *out = h;
    return ESP_OK;
}
...
{
    return (capilot_bsp_i2c_bus_handle_t)s_bus_handle;
}

/* 通用 I2C 读写：供 capilot_imu / capilot_audio 等组件复用同一总线 */
esp_err_t capilot_bsp_i2c_read(uint8_t dev_addr, uint8_t reg, uint8_t *data, size_t len)
{
    ...
    ESP_RETURN_ON_ERROR(
        i2c_get_cached_dev(dev_addr, &dev),
        TAG, "get I2C dev handle failed");
    return i2c_master_transmit_receive(
        dev, &reg, 1, data, len, 1000 / portTICK_PERIOD_MS);
}
...
esp_err_t capilot_bsp_i2c_write_byte(uint8_t dev_addr, uint8_t reg, uint8_t val)
{
    ...
    return i2c_master_transmit(
        dev, buf, 2, 1000 / portTICK_PERIOD_MS);
}
...
static esp_err_t pca9557_read_reg(uint8_t reg, uint8_t *data, size_t len)
{
    ...
    return i2c_master_transmit_receive(
        s_pca9557_dev, &reg, 1, data, len, 1000 / portTICK_PERIOD_MS);
}
...
static esp_err_t pca9557_write_reg(uint8_t reg, uint8_t data)
{
    ...
    return i2c_master_transmit(
        s_pca9557_dev, buf, 2, 1000 / portTICK_PERIOD_MS);
}
...
{
    if (!s_bus_handle) {
        ESP_LOGE(TAG, "I2C bus not initialized, skip PCA9557 init");
        return;
    }

    ESP_LOGI(TAG, "PCA9557 init (addr=0x%02X)", CAPILOT_BSP_PCA9557_ADDR);

    /* 创建 PCA9557 设备句柄（首次调用时） */
    if (!s_pca9557_dev) {
        const i2c_device_config_t dev_cfg = {
            .dev_addr_length = I2C_ADDR_BIT_LEN_7,
            .device_address  = CAPILOT_BSP_PCA9557_ADDR,
            .scl_speed_hz   = CAPILOT_BSP_I2C_FREQ_HZ,
        };
        esp_err_t ret = i2c_master_bus_add_device(
            s_bus_handle, &dev_cfg, &s_pca9557_dev);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "PCA9557 device init failed: %s", esp_err_to_name(ret));
            return;
        }
    }
...
esp_err_t capilot_bsp_touch_init(esp_lcd_touch_handle_t *ret_touch)
{
    ...
    /* 使用新 I2C Master API 的 bus handle（已迁移，不再用 legacy I2C 驱动） */
    ...
    ESP_RETURN_ON_ERROR(
        esp_lcd_new_panel_io_i2c((esp_lcd_i2c_bus_handle_t)s_bus_handle,
                                  &tp_io_config, &tp_io),
